...
@section('content')
    <div class="register-container">
        <form class="register-form p-3 rounded"
              method="post" action="{{ \App\Helpers\Helper::route('user_registration.register_identify_post') }}">
            <h4 class="text-center">{{ \App\Helpers\Helper::__('registration.user_identify_header') }}</h4>
            ...
            <div class="form-group">
                <label for="identification_id">
                    {{ \App\Helpers\Helper::__('registration.user_identify_identification_id_label') }}
                </label>
                ...
                <small id="identification_id_hint" class="form-text text-muted">
                    {{ \App\Helpers\Helper::__('registration.user_identify_identification_id_hint') }}
                </small>
            </div>
            <div class="form-group">
                <label for="date_of_birth">
                    {{ \App\Helpers\Helper::__('registration.user_identify_date_of_birth_label') }}
                </label>
                <input type="date" class="form-control @error('date_of_birth') is-invalid @enderror"
                       id="date_of_birth" name="date_of_birth"
                       required max="{{ \Carbon\Carbon::now()->toDateString() }}"
                ...
                <small id="date_of_birth_hint" class="form-text text-muted">
                    {{ \App\Helpers\Helper::__('registration.user_identify_date_of_birth_hint') }}
                </small>
            </div>
            <a class="btn btn-primary" role="button"
               href="{{ \App\Helpers\Helper::route('user_authentication.login_get') }}">
                <i class="fas fa-arrow-left"></i> {{ \App\Helpers\Helper::__('common.back_to_login') }}
            </a>
            <button type="submit" class="btn btn-primary">{{ \App\Helpers\Helper::__('common.next') }}</button>
            @csrf
        </form>
    </div>
@endsection
