...
@section('content')
    <div class="register-container">
        <form class="register-form p-3 rounded"
              method="post" action="{{ route('user_registration.register_verify_post') }}">
            <h4 class="text-center">{{ __('registration.user_verify_header') }}</h4>
            ...
            <button type="submit" class="btn btn-primary btn-block">{{ __('common.verify') }}</button>
            <a class="btn btn-primary btn-block" role="button"
               href="{{ route('user_registration.register_identify_get') }}">
